# Bug Lane — Migration 109 (apply to Supabase)

**What:** Adds the `bugs` table (per-project bug reports) + RLS + a promotion-integrity guard. Backs the new **Bugs** lane on the Dev Project detail page.

**When:** Apply this **before** merging/deploying the `feat/project-bug-lane` frontend, or the Bugs lane will render empty and `useBugs` will log `bugs fetch failed` in the console.

**How:**
1. Open the [Supabase SQL Editor](https://supabase.com/dashboard/project/_/sql/new) for the project-engine database.
2. Paste the whole block below and **Run**.
3. It's idempotent / re-run-safe (drop-if-exists guards), so re-running is harmless.

> Depends on `is_project_member()` (migration 106) and `projects` / `tasks.project_id` (106–107) — all already live in prod.
...
-- normal project-tagged fix task (urgency from severity, bug icon) and links
...
1. Open a Dev Project → a third **Bugs** section appears under Feature Requests.
2. Report a bug (with a severity) → it lands in the **Reported** column with a severity chip.
3. Drag **Reported → Confirmed** (Board view) → status persists on refresh.
4. Open the card → change severity/status → **Save**.
5. **Promote** a *Critical* bug → a fix task opens in the panel with urgency **Urgent** + 🐛 icon; the bug moves to **Promoted**; the task also shows in the Features lane and in My Tasks.
6. A non-member of the project cannot see its bugs (RLS).
