    ...
    <div className="space-y-5">
      ...
      <div className="card overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {data.map(r => (
              <tr key={r.name} className={`border-b border-slate-100 dark:border-dark-border ${r.completionRate < 50 && r.received > 2 ? 'bg-red-500/5' : ''}`}>
                ...
                <td className="table-td text-center">
                  {r.declineRate > 0 && (
                    <span className={`badge ${r.declineRate > 30 ? 'bg-red-500/15 text-red-700' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300'}`}>
                      {r.declineRate}%
                    </span>
                  )}
                </td>
            ...
            <tbody>
              {allDeclined.map(t => (
                <tr key={t.id} className="border-b border-slate-100 dark:border-dark-border">
                  ...
                  <td className="table-td text-slate-500 dark:text-slate-400">{t.assigner?.full_name}</td>
                  <td className="table-td text-xs text-slate-500 dark:text-slate-400 italic max-w-[200px] truncate">
                    {t.decline_reason || '—'}
                  </td>
                  <td className="table-td text-xs text-slate-400 dark:text-slate-500">
