    ...
    <div className="space-y-5">
      ...
      <div className="card overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {data.map(r => (
              <tr key={r.name} className={`border-b border-navy-100/20 ${r.completionRate < 50 && r.received > 2 ? 'bg-red-500/5' : ''}`}>
                <td className="table-td font-medium">{r.name}</td>
                <td className="table-td text-navy-500">{r.team}</td>
                ...
                <td className="table-td text-center">
                  {r.declineRate > 0 && (
                    <span className={`badge ${r.declineRate > 30 ? 'bg-red-500/15 text-red-700' : 'bg-navy-100/50 text-navy-600'}`}>
                      {r.declineRate}%
                    </span>
                  )}
                </td>
                <td className="table-td text-xs text-navy-500">
                  {r.longestTask !== '—' && <span>{r.longestTask} <span className="text-orange-500">({r.longestDays}d)</span></span>}
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {allDeclined.length > 0 && (
        <div className="card">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {allDeclined.map(t => (
                <tr key={t.id} className="border-b border-navy-100/20">
                  <td className="table-td">
                    <div className="font-medium">{t.title}</div>
                    <div className="text-xs text-navy-400">{t.task_id}</div>
                  </td>
                  <td className="table-td text-navy-500">{t.assignee?.full_name}</td>
                  <td className="table-td text-navy-500">{t.assigner?.full_name}</td>
                  <td className="table-td text-xs text-navy-500 italic max-w-[200px] truncate">
                    {t.decline_reason || '—'}
                  </td>
                  <td className="table-td text-xs text-navy-400">
