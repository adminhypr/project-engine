  ...
  return (
    <div>
      ...
      <div className="card overflow-x-auto">
        <p className="text-xs text-slate-500 dark:text-slate-400 mb-3">Rows = who assigned, Columns = who received. Darker = more tasks.</p>
        {names.length === 0 ? (
          <p className="text-center py-8 text-slate-400 dark:text-slate-500 text-sm">No assignment data for this filter.</p>
        ) : (
          <table className="text-xs border-collapse">
            <thead>
              <tr>
                <th className="table-th sticky left-0 bg-white dark:bg-dark-card min-w-[120px]">Assigner ↓ / Assignee →</th>
            ...
            <tbody>
              {matrix.map(row => (
                <tr key={row.assigner}>
                  <td className="table-td sticky left-0 bg-white dark:bg-dark-card font-medium">{row.assigner}</td>
                  {assignees.map(a => {
                    const val     = row[a] || 0
