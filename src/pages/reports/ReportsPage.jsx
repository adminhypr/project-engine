  ...
  return (
    <PageTransition>
      <div className="flex flex-col md:flex-row h-full">
        ...
        <aside className="md:w-52 md:min-w-[13rem] border-b md:border-b-0 md:border-r border-slate-100 dark:border-dark-border bg-white dark:bg-dark-surface py-2 md:py-4">
          <p className="px-4 text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2 hidden md:block">Reports</p>
            ...
            {visibleReports.map(r => (
              <button
                ...
                className={`relative whitespace-nowrap text-left px-3 md:px-4 py-2 md:py-2.5 text-sm rounded-lg md:rounded-none transition-all duration-200
                  ${activeReport === r.id
                    ? 'text-brand-600 dark:text-brand-300 font-semibold bg-brand-50 dark:bg-brand-500/15 md:bg-transparent md:dark:bg-transparent'
                    : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-dark-hover'}`}
              >
                {activeReport === r.id && (
                  <motion.div
                    layoutId="report-active"
                    className="absolute inset-0 hidden md:block bg-brand-50 dark:bg-brand-500/15 border-r-2 border-brand-500"
            ...
            actions={
              <div className="flex items-center gap-2 sm:gap-3">
                <div className="flex items-center gap-1.5 sm:gap-2 text-sm">
                  <label className="text-slate-500 dark:text-slate-400 font-medium hidden sm:inline">From</label>
                  ...
                  <label className="text-slate-500 dark:text-slate-400 font-medium hidden sm:inline">To</label>
