    ...
    <div className="relative" ref={panelRef}>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            {/* Header */}
            <div className="px-4 py-3 border-b border-navy-100/20 flex items-center justify-between">
            ...
            <div className="max-h-[400px] overflow-y-auto">
              {notifications.length === 0 ? (
                <div className="py-8 text-center">
                  <Bell size={24} className="text-navy-300 mx-auto mb-2" />
                  <p className="text-sm text-navy-400">All caught up!</p>
                </div>
              ) : (
                notifications.map((n, i) => (
                  <motion.div
                    key={n.id}
                    className="px-4 py-3 border-b border-navy-100/10 hover:bg-navy-50/50 transition-colors cursor-pointer flex gap-3"
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold text-navy-700">{n.title}</p>
                      <p className="text-xs text-navy-500 truncate">{n.body}</p>
                      <p className="text-[10px] text-navy-400 mt-0.5">{timeAgo(n.time)}</p>
                    </div>
                    <button
                      onClick={(e) => { e.stopPropagation(); dismiss(n.id) }}
                      className="p-1 rounded-lg text-navy-300 hover:text-navy-600 hover:bg-navy-100/50 transition-colors flex-shrink-0"
                    >
                      <X size={12} />
                    </button>
                  </motion.div>
